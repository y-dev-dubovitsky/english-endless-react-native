...
const TENSE_COLORS = {
  ...
  PAST_SIMPLE: ['#FFA500', '#FFB74D'], // оранжевые оттенки
  PAST_CONTINUOUS: ['#6C5CE7', '#8175EA'],
  PAST_PERFECT: ['#FD79A8', '#FF8DB8'],
  FUTURE_SIMPLE: ['#00BFFF', '#1E90FF'], // голубые оттенки
  FUTURE_CONTINUOUS: ['#00CED1', '#20B2AA'], // голубые оттенки
  FUTURE_PERFECT: ['#E84393', '#F368A6'],
};

const TENSES_LIST = [
  {
    entity: PRESENT_SIMPLE,
    type: 'PRESENT_SIMPLE',
    title: 'Present Simple',
    description: 'Факты, регулярные действия',
  },
  {
    entity: PRESENT_CONTINUOUS,
    type: 'PRESENT_CONTINUOUS',
    title: 'Present Continuous',
    description: 'Действия в процессе',
  },
  {
    entity: PRESENT_PERFECT,
    type: 'PRESENT_PERFECT',
    title: 'Present Perfect',
    description: 'Опыт и результат',
  },
  {
    entity: PAST_SIMPLE,
    type: 'PAST_SIMPLE',
    title: 'Past Simple',
    description: 'Завершенные действия в прошлом',
  },
  {
    entity: PAST_CONTINUOUS,
    type: 'PAST_CONTINUOUS',
    title: 'Past Continuous',
    description: 'Длительные действия в прошлом',
  },
  {
    entity: PAST_PERFECT,
    type: 'PAST_PERFECT',
    title: 'Past Perfect',
    description: 'Предпрошедшее время',
  },
  {
    entity: FUTURE_SIMPLE,
    type: 'FUTURE_SIMPLE',
    title: 'Future Simple',
    description: 'Будущие действия и обещания',
  },
  {
    entity: FUTURE_CONTINUOUS,
    type: 'FUTURE_CONTINUOUS',
    title: 'Future Continuous',
    description: 'Длительные действия в будущем',
  },
  {
    entity: FUTURE_PERFECT,
    type: 'FUTURE_PERFECT',
    title: 'Future Perfect',
    description: 'Завершенные действия к моменту',
  },
];
...
const TenseScreen = (props: any): JSX.Element => {
  ...
  const orbColor1 = useMemo(
    () =>
      theme === 'dark' ? 'rgba(102, 61, 255, 0.1)' : 'rgba(99, 102, 241, 0.1)',
    [theme]
  );

  const orbColor2 = useMemo(
    () =>
      theme === 'dark' ? 'rgba(0, 212, 170, 0.1)' : 'rgba(16, 185, 129, 0.1)',
    [theme]
  );
  ...
  const cardsScale = React.useRef(
    TENSES_LIST.map(() => new Animated.Value(0.9))
  ).current;
  const cardsOpacity = React.useRef(
    TENSES_LIST.map(() => new Animated.Value(0))
  ).current;
  const cardsTranslate = React.useRef(
    TENSES_LIST.map(() => new Animated.Value(30))
  ).current;
  ...
  const TenseCard = React.useCallback(
    ({ tense, index }: { tense: any; index: number }) => {
              ...
              <View style={styles.cardContent}>
                <View style={styles.cardHeader}>
                  <View style={styles.iconContainer}>
                    <Text style={styles.icon}>{getTenseIcon(tense.type)}</Text>
                  </View>
